...
import { handleSiprovQueue } from "./handleSiproveQueue";
import { debounce } from "../../helpers/Debounce";
import { ChatCompletionRequestMessage, Configuration, OpenAIApi } from "openai";
import notificationOfflineCarService from '../NotificationOfflineCarService/NotificationOfflineCarService'
...
import FlowBot from "../FlowBot/conectFlowService";
import smartGpsService from "../SmartGpsService/smartGpsService";
import ListUserQueueServices from "../UserServices/ListUserQueueServices";
...
const verifyQueue = async (
  ...
  if (queues.length === 1) {
    ...
    //inicia integração dialogflow/n8n/smartgps
    if (
      !msg.key.fromMe &&
      !ticket.isGroup &&
  ...
  if (choosenQueue) {
    ...
    let chatbot = false;
    let typeIntegration = ''
    // if (choosenQueue?.integrationId && companyId) {
    //   const integrations = await ShowQueueIntegrationService(choosenQueue?.integrationId, companyId);
    //   typeIntegration = integrations?.type
    // }
    if (choosenQueue?.options && typeIntegration !== 'smartgps') {
      chatbot = choosenQueue.options.length > 0;
    }
    if (typeIntegration === 'smartgps') {
      chatbot = true;
    }
    ...
    if (ticketTraking.chatbotAt !== null) {
      ...
      console.log({ dataLimite, Agora, chatbotAt: ticketTraking.chatbotAt })
...
const handleChartbot = async (ticket: Ticket, msg: WAMessage, wbot: Session, dontReadTheFirstQuestion: boolean = false) => {
  ...
  // voltar para o menu anterior vvq
  ...
  if (!isNil(queue) && isNil(ticket.queueOptionId)) {
    ...
    const botButton = async () => {
      ...
      //console.log({ticketIntegrationId3: ticket?.integrationId})
      //const integrations = await ShowQueueIntegrationService((ticket?.integrationId || 2), companyId);
      // if (integrations.type === "smartgps") {
      //   buttons.push({
      //     buttonId: `*`,
      //     buttonText: { displayText: "Menu inicial da smartgps" },
      //     type: 4
      //   });
      // } else {
      //   buttons.push({
    ...
    const botText = async () => {
      ...
      //console.log({ticketIntegrationId4: ticket?.integrationId})
      //const integrations = await ShowQueueIntegrationService((ticket?.integrationId || 2), companyId);
      // if (integrations.type === "smartgps") {
      //   options += `\n*[ * ]* - Menu inicial da smartgps`;
      // } else {
      //   options += `\n*[ 0 ]* - Menu anterior`;
      //   options += `\n*[ # ]* - Menu inicial`;
      // }
    ...
    if (queueOptions.length > -1) {
      ...
      const botList = async () => {
        ...
        //console.log({ticketIntegrationId5: ticket?.integrationId})
        //  const integrations = await ShowQueueIntegrationService((ticket?.integrationId || 2), companyId);
        // if (integrations.type === "smartgps") {
        //   sectionsRows.push({
        //     title: "Menu inicial da smartgps",
        //     rowId: `*`
        //   });
        //   const sections = [
        //     {
      ...
      const botButton = async () => {
        ...
        //console.log({ticketIntegrationId1: ticket?.integrationId})
        //const integrations = await ShowQueueIntegrationService((ticket?.integrationId || 2), companyId);
        // if (integrations.type === "smartgps") {
        //   buttons.push({
        //     buttonId: `*`,
      ...
      // adionar logica pra tratar opcoes
      const botText = async () => {
        ...
        //console.log({ticketIntegrationId2: ticket?.integrationId})
        //const integrations = await ShowQueueIntegrationService((ticket?.integrationId || 2), companyId);
        // if (integrations.type === "smartgps") {
        //   options += `\n*[ * ]* - Menu inicial da smartgps`;
        // } else {
      ...
      //ALTERAÇÃO PARA ENVIO DE FILA CHATBOT
...
export const handleMessageIntegration = async (
  ...
  } else if (queueIntegration.type === "smartgps") {
    console.log("entrou no smartgps")
    await smartGpsService({ ticket, msg, wbot, smartgps: queueIntegration });
  } else if (queueIntegration.type === "smartgpsOfflineCar") {
    console.log("entrou no smartgpsOfflineCar")
    await smartGpsService({ ticket, msg, wbot, smartgps: queueIntegration });
  } else if (queueIntegration.type === "flowbuilder") {
    console.log("entrou no flowbuilder")
    if (ticket?.user_api_hash !== 'transferQueue') {
      await FlowBot({ ticket, msg, wbot, queueIntegration });
    }
...
const handleMessage = async (
  ...
  try {
    ...
    if (unreadMessages === 0 && whatsapp.complationMessage && formatBody(whatsapp.complationMessage, ticket).trim().toLowerCase() === lastMessage?.body.trim().toLowerCase()) {
      return;
    }
    ...
    const settings = await Setting.findAll({
      where: {
        key: ["siprov", "siprovPass"],
        companyId
      }
    });

    // Mapeia as configurações para um objeto chave-valor
    ...
    // Extrai os valores
    const siprovUser = settingsMap["siprov"];
    const siprovPass = settingsMap["siprovPass"];

    if (!isNil(siprovUser) && !isNil(siprovPass)) {
      await handleSiprovQueue(ticket, msg, companyId, contact, wbot as WASocket, siprovUser, siprovPass);
    }
    await ticket.update({
      whatsappId: wbot.id,
    });
    ...
    if (
      ...
      !msg.key.fromMe &&
      !ticket.userId &&
      !isNil(whatsapp.flowId)
    ) {
      if (ticket?.user_api_hash !== 'transferQueue') {
        await FlowBot({ ticket, msg, wbot, flowId: whatsapp.flowId });
      }
    }
...
const verifyCampaignMessageAndCloseTicket = async (
  ...
  if (message.key.fromMe && isCampaign) {
    ...
    await ticket.update({ status: "closed", user_api_hash: null });
  ...
  try {
    ...
    wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
      ...
      messages.forEach(async (message: proto.IWebMessageInfo) => {
        ...
        if (!message.key.fromMe) {
          const number = message.key.remoteJid.split('@')[0].replace(/[^0-9]/g, "")
          // console.log('Buscar contatos....')
          const cntt = await Contact.findOne({
            where: {
              number: `${number}`,
              companyId,
            }
          ...
          if (cntt) {
            ...
            const tickt = await Ticket.findOne({
              where: {
                contactId: cntt.id,
                status: "chatbot",
                companyId,
            ...
            // console.log('Contato encontrado.', `${number}`)
            ...
            if (tickt) {
              console.log('ticket 1 encontrado.')

              const regex = /^(?:[1-8])$/;
              const uniqueCaracter = message.message?.conversation.length < 2 ? true : false;
              const response = regex.test(message.message?.conversation) && uniqueCaracter ? message.message.conversation : "8";

              if (tickt && response === '8') {
                console.log('ticket encontrado mas movido pra falar com atendente.')
                wbot.sendMessage(
                  message.key.remoteJid,
                  {
                    text: 'Estamos encaminhando você para falar com nossos atendentes.'
                  }
                )
                await UpdateTicketService({
              ...
              if (!jsonData?.integration) {
                ...
                wbot.sendMessage(
                  message.key.remoteJid,
                  {
                    text: 'Erro em processar sua reposta, aguarde o atendente.'
                  }
                )
                return;
              }
              if (response === "5" || response === "7") {
                const res = await notificationOfflineCarService({
                ...
                if (res) {
                  wbot.sendMessage(
                    ...
                    {
                      text:
                        jsonData.integration.data?.text
                          ? jsonData.integration.data.text
                          : 'Obrigada pelas informações descritas, atualizamos no nosso sistema o status do seu chamado.'
                    }
                  )
                  await UpdateTicketService({
                    ticketData: { status: "closed", user_api_hash: null },
              ...
              wbot.sendMessage(
                message.key.remoteJid,
                {
                  text: '🗓️Gentileza nos informar a previsão de dias que o veículo irá permanecer nessa situação.'
                }
              )
              const convertToStr = JSON.stringify({
                ...jsonData,
                response,
              })
              ...
              console.log('atualizado pra segunda msg')
              console.log({ novoStatus: tickt.status })
              // console.log({
              //   user_api_hash: convertToStr,
              // })
              return;
            ...
            if (ticket2) {
              console.log('chat segunda msg')
              const jsonData = JSON.parse(ticket2.user_api_hash)
              ...
              const regex = /^\d+$/;
              const response = regex.test(message.message?.conversation) ? message.message.conversation : "1";
              console.log('chamando a api externa')
              console.log({
                data: jsonData.integration.data,
                type: jsonData.integration.type,
                externalUrl: jsonData.externalUrl,
          ...
          // console.log('Contato nao encontrado.', `${number}`)
...
export { wbotMessageListener, handleMessage, verifyContact };
