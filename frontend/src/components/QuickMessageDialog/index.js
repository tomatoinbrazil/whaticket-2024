...
const QuickMessageDialog = ({ open, onClose, quickemessageId, reload }) => {
    ...
    const handleSaveQuickeMessage = async (values) => {
        ...
        const quickemessageData = { ...values, isMedia: true, mediaPath: attachment ? String(attachment.name).replace(/ /g, "_") : values.mediaPath ? values.mediaPath.replace(/ /g, "_") : null };
