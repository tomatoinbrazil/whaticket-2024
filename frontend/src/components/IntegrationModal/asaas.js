...
const AsaasIntegrationModal = ({ open, onClose, integrationId }) => {
    ...
    const [asaas, setAsaas] = useState(initialState);
    ...
    /*     useEffect(() => {
            const fetchSession = async () => {
                try {
                    const { data } = await api.get(`/integration/asaas`);
                    setResp(data);
                } catch (err) {
                    //toastError(err);
                }
    ...
    useEffect(() => {
        const fetchIntegration = async () => {
            try {
                const { data } = await api.get(`/integration`, { params: { name: "asaas" } });
                ...
                setAsaas({ hora, token, envioAnt, envioAposVenc, msgAntVenc, msgVenc, msgAposVenc, whatsappId });
            } catch (err) {
                //toastError(err);
            }
        };
...
export default React.memo(AsaasIntegrationModal);
