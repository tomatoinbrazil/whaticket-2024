...
const CreateCompanyService = async (
  ...
  // Configuração de "sendSignMessage" 
  await Setting.findOrCreate({
    where: {
      companyId: company.id,
      key: "sendSignMessage"
  ...
  // Configuração de "sendGreetingMessageOneQueues" 
  await Setting.findOrCreate({
    where: {
      companyId: company.id,
  ...
  // Configuração de "sendMsgTransfTicket" 
  await Setting.findOrCreate({
    where: {
      companyId: company.id,
  ...
  // Configuração de "sendFarewellWaitingTicket" 
  await Setting.findOrCreate({
    where: {
      companyId: company.id,
  ...
  // Configuração de "sendQueuePosition" 
  await Setting.findOrCreate({
    where: {
      companyId: company.id,
