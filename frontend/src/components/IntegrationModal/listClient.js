...
const ListClienteIntegrationModal = ({ open, onClose, integrationId, response }) => {
    ...
    useEffect(() => {
        const fetchIntegration = async () => {
            try {
                const data = await api.get(`/integration/asaas`).then(res => {
                    setResp(res.data);
                });
                await delay(4000);
                setLoading(false)
    ...
    useEffect(() => {
        const fetchIntegration = async () => {
            try {
                const data = await api.get(`/integration`, { params: { name: "asaas" }}).then(res => {
    ...
    return (
        <div className={classes.root}>
            ...
            >
                ...
                />
                <DialogTitle>
                    {integrationId ? i18n.t("integrationModal.title.edit") : i18n.t("integrationModal.title.add")}
                </DialogTitle>
                <Formik
