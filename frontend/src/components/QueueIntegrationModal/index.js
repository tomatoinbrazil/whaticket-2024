...
const QueueIntegration = ({ open, onClose, integrationId }) => {
  ...
  const handleSaveDialogflow = async (values) => {
    try {
      if (values.type === 'n8n' || values.type === 'webhook' || values.type === 'typebot' || values.type === 'smartgps' || values.type === 'flowbuilder') values.projectName = values.name
      if (integrationId) {
                  ...
                  <Grid container spacing={1}>
                    <Grid item xs={12} md={6} xl={6}>
                      ...
                      >
                        ...
                        >
                          ...
                          <MenuItem value="smartgps">SmartGps</MenuItem>
                          <MenuItem value="flowbuilder">FlowBuilder</MenuItem>
                          {/* <MenuItem value="smartgpsAutoChat">SmartGpsAutoChat</MenuItem> */}
                        </Field>
                      </FormControl>
                    </Grid>
                    ...
                    {(values.type === "flowbuilder") && (
                      <>
                        ...
                        >
                          {
                            flows && flows.length > 0 && flows.map(flw =>
                              (<MenuItem value={flw.id}>{flw.title}</MenuItem>)
                            )
                          }
                         </Field>
                      </>
                    )}
                    {/* {(values.type === "smartgpsAutoChat") && (
                      <>
                        <Grid item xs={12} md={6} xl={6} >
                          <Field
                            as={TextField}
                            label={i18n.t("queueIntegrationModal.form.numberDays")}
                            autoFocus
                            required
                            name="name"
                            error={touched.name && Boolean(errors.name)}
