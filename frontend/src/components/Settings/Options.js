...
export default function Options(props) {
  ...
  const [loadingSiprovType, setLoadingSiprovType] = useState(false);
  ...
  useEffect(() => {
    if (Array.isArray(settings) && settings.length) {
      ...
      const siprovType = settings.find((s) => s.key === "siprov");
      if (siprovType) {
        setSiprovType(siprovType?.value);
      }

      const siprovPassType = settings.find((s) => s.key === "siprovPass");
      if (siprovPassType) {
        setSiprovPassType(siprovPassType?.value);
      }
    }
  ...
  async function handleChangeSiprov(value) {
    setSiprovType(value);
    setLoadingSiprovType(true);
    await update({
      key: "siprov",
      value,
    });
    toast.success("Operação atualizada com sucesso.");
    setLoadingSiprovType(false);
  }

  async function handleChangeSiprovPass(value) {
    setSiprovPassType(value);
        ...
        <Grid xs={12} sm={6} md={6} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              value={siprovType}
              onChange={async (e) => {
                handleChangeSiprov(e.target.value);
              }}
            >
            </TextField>
            <FormHelperText>
        ...
        <Grid xs={12} sm={6} md={6} item>
          <FormControl className={classes.selectContainer}>
            <TextField
              ...
              onChange={async (e) => {
                handleChangeSiprovPass(e.target.value);
              }}
            >
            </TextField>
            <FormHelperText>
