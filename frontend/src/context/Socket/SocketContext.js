...
const SocketManager = {
  ...
  getSocket: function (companyId) {
    ...
    if (companyId !== this.currentCompanyId || userId !== this.currentUserId) {
      ...
      if (!token) {
        console.warn("No token found, returning DummySocket");
        return new DummySocket();
      }
      ...
      console.log("Connecting to WebSocket server at:", process.env.REACT_APP_BACKEND_URL);
      ...
      this.currentSocket.on("connect_timeout", () => {
