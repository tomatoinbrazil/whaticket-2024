...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	const initialcurrentRecurrence = {
		id: null,
	...
	const [currentRecurrence, setCurrentRecurrence] = useState(initialcurrentRecurrence);
		...
		if (open) {
			try {
				(async () => {
					...
					setCurrentRecurrence(recurrenceData.find(rec => rec.id === data.recurrence));
	...
	}, [scheduleId, contactId, open, user, recurrenceData]);
								...
								<div className={classes.multFieldLine}>
									...
									>
										<Autocomplete
											...
											onChange={(e, strCurrence) => {
												const recurrence = strCurrence ? strCurrence.id : '';
												setSchedule({ ...schedule, recurrence });
												setCurrentRecurrence(strCurrence ? strCurrence : initialContact);
											}}
											getOptionLabel={(option) => option.name}
											getOptionSelected={(option, value) => {
												return value.id === option.id
											}}
											renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Selecione a recorrencia (Opcional)" />}
										/>
									</FormControl>
								</div>
								<br />
								{
									currentRecurrence && currentRecurrence?.id && (
										<div>
											<Field
												as={TextField}
												rows={9}
